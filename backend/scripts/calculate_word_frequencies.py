...
def import_word_counts():
    ...
    with codecs.open("../data/tatoeba/word_counts2.csv", 'r', encoding="utf-8") as f:
...
def store_word_counts(df):
    ...
    df.to_csv('../data/tatoeba/word_counts2.csv', index=False)
...
def calculate_and_store_sentence_ease(df):
    ...
    df.to_csv('../data/tatoeba/sentence_ease_pl2.csv', index=False)
...
def loop_over_tatoeba():
    ...
    df_polish_sentences_2 = df_polish_sentences.drop_duplicates(['sentenceID'])
    for index, row in df_polish_sentences_2.iterrows():
        ...
        df_polish_sentences_2.at[index, 'words_in_sentence'] = stripped_sentence_set
    ...
    calculate_and_store_sentence_ease(df_polish_sentences_2)
