...
def calculate_and_store_sentence_ease(df):
    ...
    for index, row in df.iterrows():
        sentence_summed_count = sum([df_word_counts.loc[df_word_counts['word'] == word, 'count'] for word in row['words_in_sentence']])
        sentence_length = len(row['words_in_sentence'])
        df.at[index, 'sentence_ease'] = sentence_summed_count/sentence_length
