...
CREATE_PERSONAL_TABLE_PL = "CREATE TABLE IF NOT EXISTS personal_ease_pl (sentenceID INTEGER, sentence_pl TEXT," \
                        "sentence_en TEXT, sentence_nl TEXT, lang TEXT, words_in_sentence TEXT," \
                        "sentence_ease FLOAT, personal_sentence_ease FLOAT)"
UPDATE_USER_STATS_PL = "UPDATE personal_ease_pl " \
...
def initialize_table():
    ...
    with conn:
        conn.execute(CREATE_PERSONAL_TABLE_PL)
        df.to_sql(name='personal_ease_pl', con=conn, if_exists='fail', index=False)
